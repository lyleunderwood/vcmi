/*
 * LobbySetTurnTime.cpp, part of homam-web fork of VCMI engine.
 *
 * JSON codec for LobbySetTurnTime.
 *
 * C++ pack definition:  vcmi/lib/networkPacks/PacksForLobby.h
 * TypeScript twin:      wrapper/src/codecs/lobby/LobbySetTurnTime.ts
 *
 * NOTE: TurnTimerInfo is not (yet) promoted to a shared codec. Its fields
 * are inlined here. If a second pack ever references TurnTimerInfo on the
 * wire, promote to vcmi/server/json/shared/TurnTimerInfo.{h,cpp}.
 */
#include "StdInc.h"
...
class LobbySetTurnTimeCodec final : public PackCodec
{
public:
	std::string typeName() const override { return "LobbySetTurnTime"; }

	bool matches(const CPack & pack) const override
	{
		return dynamic_cast<const LobbySetTurnTime *>(&pack) != nullptr;
	}

	std::unique_ptr<CPack> fromJson(const JsonNode & json) const override
	{
		auto pack = std::make_unique<LobbySetTurnTime>();
		const JsonNode & t = json["turnTimerInfo"];
		auto & info = pack->turnTimerInfo;
	...
	{
		const auto & p = dynamic_cast<const LobbySetTurnTime &>(pack);
		const auto & info = p.turnTimerInfo;

		out["type"].String() = typeName();

		JsonNode & t = out["turnTimerInfo"];
		t.Struct();
...
REGISTER_PACK_CODEC(LobbySetTurnTimeCodec)
